...
IMAGE_PATH = os.path.join(os.path.dirname(__file__), "2026-02-10_08-53-49.png")


def test_first_four_lines():
...
def test_extracts_multiple_zones():
    """Verify that multiple zone labels are extracted."""
    lines = extract_zones(IMAGE_PATH)
    # The image contains many zone/range labels
    assert len(lines) >= 15, f"Expected at least 15 lines, got {len(lines)}"


def test_contains_support_and_resistance():
...
def test_contains_key_zone_labels():
